...
export const codexCliAdapter: AgentAdapter = {
    ...
    for (let i = 0; i < lines.length; i++) {
      try {
        ...
        if (entry.type === 'event_msg' && entry.payload?.type === 'token_count') {
          ...
          assistant.message.model = assistant.message.model || modelByTurn.get(targetTurnId) || CODEX_FALLBACK_MODEL
          continue
        }
        ...
        messages.push({
          ...
          message: {
            ...
            model: currentTurnId ? (modelByTurn.get(currentTurnId) || CODEX_FALLBACK_MODEL) : CODEX_FALLBACK_MODEL,
          },
        })
  ...
  async resumeSession(sourceSessionId: string, cwd: string | null): Promise<void> {
    ...
    launchDetached('wt', ['-d', dir, 'powershell.exe', '-c', `codex resume ${sourceSessionId}`])
  },
}